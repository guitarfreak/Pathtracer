...
void drawPoint(Vec3 p, Vec4 color) {
	glBindTexture(GL_TEXTURE_2D, 0);
	Vec4 c = COLOR_SRGB(color);
	glColor4f(c.r, c.g, c.b, c.a);
	glBegin(GL_POINTS);
...
void drawBoxQuads(Vec3 pos, Vec3 dim, Vec4 color) {
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	for(int i = 0; i < 6*5; i += 5) {
		Vec3 n = boxVertices[i+4];
		pushVec(boxVertices[i]  ); glNormal3f(n.x, n.y, n.z);
		pushVec(boxVertices[i+1]); glNormal3f(n.x, n.y, n.z);
		pushVec(boxVertices[i+2]); glNormal3f(n.x, n.y, n.z);
		pushVec(boxVertices[i+3]); glNormal3f(n.x, n.y, n.z);
	}
	glEnd();
...
void drawPlaneRaw() {
	...
	Mesh* mesh = &globalGraphicsState->meshes[MESH_PLANE];
